...
import time
import numpy as np
from annotationframeworkclient import FrameworkClient, chunkedgraph
...
def refine_skeleton_vertices(sk, cv, lvl2_ids, nan_rounds=20):
    ...
    sk : meshparty.skeleton.Skeleton
        Skeleton of level 2 ids with N vertices
    cv : cloudvolume.CloudVolume
    ...
    new_verts = np.vstack(
        [utils.get_centered_mesh(lvl2_meshes.get(k, None)) for k in lvl2_ids])
